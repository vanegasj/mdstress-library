...
namespace mds
{
    ...
    static inline void summatrix6 ( dmatrix6 a, dmatrix6 b, dmatrix6 c)
    {
        c[0][0] = a[0][0] + b[0][0]; c[0][1] = a[0][1] + b[0][1]; c[0][2] = a[0][2] + b[0][2]; c[0][3] = a[0][3] + b[0][3]; c[0][4] = a[0][4] + b[0][4]; c[0][5] = a[0][5] + b[0][5];
                                     c[1][1] = a[1][1] + b[1][1]; c[1][2] = a[1][2] + b[1][2]; c[1][3] = a[1][3] + b[1][3]; c[1][4] = a[1][4] + b[1][4]; c[1][5] = a[1][5] + b[1][5];
                                                                  c[2][2] = a[2][2] + b[2][2]; c[2][3] = a[2][3] + b[2][3]; c[2][4] = a[2][4] + b[2][4]; c[2][5] = a[2][5] + b[2][5];
                                                                                               c[3][3] = a[3][3] + b[3][3]; c[3][4] = a[3][4] + b[3][4]; c[3][5] = a[3][5] + b[3][5];
                                                                                                                            c[4][4] = a[4][4] + b[4][4]; c[4][5] = a[4][5] + b[4][5];
                                                                                                                                                         c[5][5] = a[5][5] + b[5][5];
    }
    ...
    static inline void summatrix6sq ( dmatrix6 a, dmatrix b, dmatrix6 c)
    {
        c[0][0] = a[0][0] + b[0][0]*b[0][0]; c[0][1] = a[0][1] + b[0][0]*b[1][1]; c[0][2] = a[0][2] + b[0][0]*b[2][2]; c[0][3] = a[0][3] + b[0][0]*b[1][2]; c[0][4] = a[0][4] + b[0][0]*b[0][2]; c[0][5] = a[0][5] + b[0][0]*b[0][1];
                                             c[1][1] = a[1][1] + b[1][1]*b[1][1]; c[1][2] = a[1][2] + b[1][1]*b[2][2]; c[1][3] = a[1][3] + b[1][1]*b[1][2]; c[1][4] = a[1][4] + b[1][1]*b[0][2]; c[1][5] = a[1][5] + b[1][1]*b[0][1];
                                                                                  c[2][2] = a[2][2] + b[2][2]*b[2][2]; c[2][3] = a[2][3] + b[2][2]*b[1][2]; c[2][4] = a[2][4] + b[2][2]*b[0][2]; c[2][5] = a[2][5] + b[2][2]*b[0][1];
                                                                                                                       c[3][3] = a[3][3] + b[1][2]*b[1][2]; c[3][4] = a[3][4] + b[1][2]*b[0][2]; c[3][5] = a[3][5] + b[1][2]*b[0][1];
                                                                                                                                                            c[4][4] = a[4][4] + b[0][2]*b[0][2]; c[4][5] = a[4][5] + b[0][2]*b[0][1];
    ...
    static inline void submatrix6sq ( dmatrix6 a, dmatrix b, dmatrix6 c, double s)
    {
        c[0][0] = s*(a[0][0] - b[0][0]*b[0][0]); c[0][1] = s*(a[0][1] - b[0][0]*b[1][1]); c[0][2] = s*(a[0][2] - b[0][0]*b[2][2]); c[0][3] = s*(a[0][3] - b[0][0]*b[1][2]); c[0][4] = s*(a[0][4] - b[0][0]*b[0][2]); c[0][5] = s*(a[0][5] - b[0][0]*b[0][1]);
                                                 c[1][1] = s*(a[1][1] - b[1][1]*b[1][1]); c[1][2] = s*(a[1][2] - b[1][1]*b[2][2]); c[1][3] = s*(a[1][3] - b[1][1]*b[1][2]); c[1][4] = s*(a[1][4] - b[1][1]*b[0][2]); c[1][5] = s*(a[1][5] - b[1][1]*b[0][1]);
                                                                                          c[2][2] = s*(a[2][2] - b[2][2]*b[2][2]); c[2][3] = s*(a[2][3] - b[2][2]*b[1][2]); c[2][4] = s*(a[2][4] - b[2][2]*b[0][2]); c[2][5] = s*(a[2][5] - b[2][2]*b[0][1]);
                                                                                                                                   c[3][3] = s*(a[3][3] - b[1][2]*b[1][2]); c[3][4] = s*(a[3][4] - b[1][2]*b[0][2]); c[3][5] = s*(a[3][5] - b[1][2]*b[0][1]);
                                                                                                                                                                            c[4][4] = s*(a[4][4] - b[0][2]*b[0][2]); c[4][5] = s*(a[4][5] - b[0][2]*b[0][1]);
                                                                                                                                                                                                                     c[5][5] = s*(a[5][5] - b[0][1]*b[0][1]);
    }
    ...
    static inline void copymatrix6 ( dmatrix6 a, dmatrix6 b)
    {
        b[0][0] = a[0][0]; b[0][1] = a[0][1]; b[0][2] = a[0][2]; b[0][3] = a[0][3]; b[0][4] = a[0][4]; b[0][5] = a[0][5];
                           b[1][1] = a[1][1]; b[1][2] = a[1][2]; b[1][3] = a[1][3]; b[1][4] = a[1][4]; b[1][5] = a[1][5];
                                              b[2][2] = a[2][2]; b[2][3] = a[2][3]; b[2][4] = a[2][4]; b[2][5] = a[2][5];
                                                                 b[3][3] = a[3][3]; b[3][4] = a[3][4]; b[3][5] = a[3][5];
                                                                                    b[4][4] = a[4][4]; b[4][5] = a[4][5];
                                                                                                       b[5][5] = a[5][5];
    }
    ...
    static inline void scalematrix6 ( dmatrix6 b, double a, dmatrix6 c)
    {
        c[0][0] = a * b[0][0]; c[0][1] = a * b[0][1]; c[0][2] = a * b[0][2]; c[0][3] = a * b[0][3]; c[0][4] = a * b[0][4]; c[0][5] = a * b[0][5];
                               c[1][1] = a * b[1][1]; c[1][2] = a * b[1][2]; c[1][3] = a * b[1][3]; c[1][4] = a * b[1][4]; c[1][5] = a * b[1][5];
                                                      c[2][2] = a * b[2][2]; c[2][3] = a * b[2][3]; c[2][4] = a * b[2][4]; c[2][5] = a * b[2][5];
                                                                             c[3][3] = a * b[3][3]; c[3][4] = a * b[3][4]; c[3][5] = a * b[3][5];
                                                                                                    c[4][4] = a * b[4][4]; c[4][5] = a * b[4][5];
                                                                                                                           c[5][5] = a * b[5][5];
    }
